public struct UNIQUEID
{
    ...
    public const ulong MASK_INDEX       = 0x0000ffff00000000; // 種類 No.
    public const ulong MASK_ID          = 0x00000000ffffffff; // フラッシュ ID

    private static uint flashId = 0;    // フラッシュ ID
    private ulong uniqueId;     // ユニーク ID 本体
    ...
    {
        return (int)this.uniqueId;  // 使わないらしいから適当?
    }

    public override string ToString()
    {
        return string.Format($"{(this.uniqueId)}");
    }

    // フラッシュIDの更新
    // オブジェクトが起動するたびに更新することで、プールから使い回されたものか判定する
    public ulong Update()
    {
